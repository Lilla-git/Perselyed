				...
				<main>
					<header class="tran secondary">
						<div>
							...
							<span id="balance">
								<?php
								$result = mysqli_query($conn, "SELECT SUM(amount) AS totalSum FROM transactions WHERE user_id=$user_id");
								$row = $result->fetch_assoc();
						...
						<div>
							...
							<span id="jov">
							 <?php
								$result0 = mysqli_query($conn, "SELECT SUM(amount) AS totalSum FROM transactions WHERE user_id=$user_id and type='bevetel'");
						...
						<div>
						  ...
						  <span id="kiad">
							<?php
								$result0 = mysqli_query($conn, "SELECT SUM(amount) AS totalSum FROM transactions WHERE user_id=$user_id and type='kiadas'");
								$row0 = $result0->fetch_assoc();
								$totalSum0 = $row0['totalSum'];
								echo  $totalSum0.' Ft';
							?>
					...
					<section>
						...
						<?php
						$result = mysqli_query($conn, "SELECT * FROM transactions WHERE user_id=$user_id ORDER BY id DESC ");
